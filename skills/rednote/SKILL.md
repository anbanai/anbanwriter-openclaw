...
调用 `prepare_workspace(content_type="rednote")` MCP 工具创建隔离工作目录，变量记为 `$DIR`。
...
从 `$DIR/content.md` 提取最终标题（第一行去掉 `# `），调用 `archive_workspace(content_type="rednote", name="{标题}")` 归档。归档后向用户报告完整的成果目录路径。
